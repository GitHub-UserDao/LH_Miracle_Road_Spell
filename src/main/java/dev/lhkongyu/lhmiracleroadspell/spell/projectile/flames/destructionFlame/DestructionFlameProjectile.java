package dev.lhkongyu.lhmiracleroadspell.spell.projectile.flames.destructionFlame;
...
public class DestructionFlameProjectile extends DestructionFlame {

    private static final float INCREMENTAL_DAMAGE = 1.4f;

    private static final float INCREMENTAL_RANGE = 1.5f;
...
//        }
    ...
    public void impactParticles(double x, double y, double z) {
        level().getServer().getPlayerList().getPlayers().forEach(player -> {
            ...
            int growthParticles = tickCount / 20;
            float growthRange = range * 2 * rangeAdditional;
            ...
            ((ServerLevel) level).sendParticles(player, new CommonParticleOptions(LHMiracleRoadSpellTool.RGBChangeVector3f(255,163,37), growthRange,true,ParticleRegistry.FIRE_EXPLOSION_PARTICLE.get()),true, xOld, yOld + 0.75D, zOld, 1, 0, 0, 0, 0);

            ((ServerLevel) level).sendParticles(player, new CommonParticleOptions(LHMiracleRoadSpellTool.RGBChangeVector3f(255,163,37), growthRange,ParticleRegistry.FIRE_EXPLOSION_PARTICLE.get()),true, xOld, yOld + 0.75D, zOld, 1, 0, 0, 0, 0);
        });
    }
    ...
    public void tick() {
        ...
        if (tickCount % SPEED_BOOST_INTERVAL == 0) {
            if (!level().isClientSide){
                ...
                setDamage(getDamage() * INCREMENTAL_DAMAGE);
                setRange((int) (range * INCREMENTAL_RANGE));
            }
