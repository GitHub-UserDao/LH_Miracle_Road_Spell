package dev.lhkongyu.lhmiracleroadspell.entity.spell.flames.evilGodFlame;
...
public class EvilGodFlameProjectile extends CommonMagicProjectile {
    ...
    public void tick() {
        ...
        if (target != null && target.isAlive()) {
            ...
            // 设置实体的移动方向
            setDeltaMovement(direction.scale(baseSpeed + changeSpeedIncrease)); // 调整尺度来控制速度
        ...
        if (tickCount == CHANGE_TIME && !level().isClientSide) {
            ...
            setRange(12);
            changeSpeedIncrease = 0.1f;
            setDeltaMovement(getDeltaMovement().scale(baseSpeed + changeSpeedIncrease + 1));
        }
    }
