package dev.lhkongyu.lhmiracleroadspell.entity.spell.flames.annihilatorMeteor;
...
public class AnnihilatorMeteorProjectile extends CommonMagicProjectile {
    ...
    public void tick() {
        ...
        if (tickCount == DELAY_TIME && !level().isClientSide && getOwner() instanceof Player player){
            ...
            if (LHMiracleRoadSpellTool.percentageProbability(20)) ReleaseFlamesSpellTool.createSpellSuperBigFireball(level(),player,lookVec,startPosition,false);
            else ReleaseFlamesSpellTool.createSpellBigFireball(level(),player,lookVec,startPosition,false);
